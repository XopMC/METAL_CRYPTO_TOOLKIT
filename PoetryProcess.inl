...
static metalError_t processMetalPoetryGpuSlot(const GpuRuntimeContext& context,
    ...
    while (isRun) {
        ...
        status = metal_launch("workerPoetry", context.block_number, context.block_threads,
                              ...
                              global_stride, device_dictionary_blob, device_dictionary_offsets,
...
static metalError_t processMetalPoetry() {
    ...
    for (size_t template_index = 0; template_index < g_poetry_templates.size() && isRun; ++template_index) {
        const PoetryPreparedTemplate& prepared = g_poetry_templates[template_index];
        STATUS = prepared.normalized_phrase;
        if (prepared.device.random_mode != 0u) {
            printf("[!] Poetry task %llu/%llu: words=%u missing=%u random [!]\n",
            ...
            printf("[!] Poetry task %llu/%llu: words=%u missing=%u combinations=%s [!]\n",
                   static_cast<unsigned long long>(template_index + 1u),
                   static_cast<unsigned long long>(g_poetry_templates.size()),
                   static_cast<unsigned int>(prepared.device.word_count),
                   static_cast<unsigned int>(prepared.device.wildcard_count),
        ...
        std::vector<metalError_t> statuses(g_gpu_contexts.size(), metalSuccess);
        ...
        for (size_t gpu_index = 0; gpu_index < g_gpu_contexts.size(); ++gpu_index) {
            workers.emplace_back([&, gpu_index]() {
                ...
                statuses[gpu_index] = processMetalPoetryGpuSlot(
                    g_gpu_contexts[gpu_index], prefixes[gpu_index], global_stride,
                    prepared, g_poetry_dictionary);
            });
        }
        ...
        for (size_t gpu_index = 0; gpu_index < statuses.size(); ++gpu_index) {
            if (statuses[gpu_index] != metalSuccess) {
                fprintf(stderr, "[!] Poetry task failed on GPU %d: %s [!]\n",
