...
kernel void workerPRIV_swap(device bool* isResult [[buffer(0)]],
    ...
    uchar seed[32];
    if (!priv_gen_read_indexed_key(lines, indexes, indexes_size, tid, config, seed)) {
        return;
    }
    ...
    for (int phase = 0; phase < int(PRIV_ROTATE_PHASES_METAL); ++phase) {
        ...
        priv_gen_process_rounds(isResult, buffResult, prvKeys, int(PRIV_ROTATE_BATCH_METAL), round, precPtr,
                                precPitch, false, 0ul, nullptr, false, true, config, filters,
