#include "WorkerBip32Common.metalh"

kernel void workerBip32_seq(device bool* isResult [[buffer(0)]],
                            device bool* buffResult [[buffer(1)]],
                            constant secp256k1_ge_storage* precPtr [[buffer(2)]],
                            constant ulong& precPitch [[buffer(3)]],
                            const device uint* d_derivations [[buffer(4)]],
                            const device uint* derindex [[buffer(5)]],
                            constant uint& der_indexes_size [[buffer(6)]],
                            constant uint& der_start_index [[buffer(7)]],
                            constant int& mode [[buffer(8)]],
                            const device uchar* start_point_dev [[buffer(9)]],
                            constant int& min_len [[buffer(10)]],
                            constant uchar& bip_mode [[buffer(11)]],
                            constant uint& iter [[buffer(12)]],
                            constant ulong& round [[buffer(13)]],
                            const device Bip32RuntimeBuffers& runtime [[buffer(14)]],
                            uint tid [[thread_position_in_grid]]) {
    FoundBuffers found;
    if (!bip32_make_found_from_runtime(found, runtime)) {
        return;
    }
    bip32_worker_seq(isResult, buffResult, precPtr, precPitch, d_derivations, derindex,
                     der_indexes_size, der_start_index, mode, start_point_dev, min_len,
                     bip_mode, iter, round, tid, *runtime.config, *runtime.filters,
                     *runtime.filterStorage, runtime.bloomStorage, runtime.xorStorage,
                     runtime.xorUnStorage, runtime.xorUcStorage, runtime.xorHcStorage,
                     found, runtime.substratePaths);
}
