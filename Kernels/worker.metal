...
kernel void worker(device bool* isResult [[buffer(0)]],
    ...
    (void)dub_mnem;
    if (tid >= indexes_size || iterations == nullptr) {
        return;
    }
    ...
    char base[512] = { 0 };
    ulong base_len = 0ul;
    if (!worker_read_indexed_line(base, base_len, lines, indexes, indexes_size, tid)) {
        return;
    }

    for (uint num = 0u; num < iterations_size; ++num) {
        char toHash[512] = { 0 };
        worker_copy_thread_char(toHash, base, uint(base_len));
        ulong len = base_len;
