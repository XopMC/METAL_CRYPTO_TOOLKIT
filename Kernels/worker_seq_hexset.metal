...
kernel void worker_seq_hexset(device bool* isResult [[buffer(0)]],
    ...
    for (uint num = 0u; num < iterations_size; ++num) {
        char toHash[512] = { 0 };
        if (!worker_hexset_materialize_candidate_exact(hexset_start_digits, hexset_size * 2u,
                                                       hexset_alphabet, hexset_base, starter,
                                                       reinterpret_cast<thread uchar*>(toHash))) {
            return;
        }
        if (!worker_hexset_candidate_within_exact_bounds(reinterpret_cast<thread uchar*>(toHash),
                                                         hexset_lower_exact, hexset_upper_exact,
                                                         hexset_size)) {
            return;
        }
