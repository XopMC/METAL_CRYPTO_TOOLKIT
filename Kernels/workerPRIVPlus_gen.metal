...
kernel void workerPRIVPlus_gen(device bool* isResult [[buffer(0)]],
    ...
    uchar seed_base[32];
    priv_gen_seed_from_entropy(seed_base, seed_l, is_64, entropy_len, mode, gen, config.skip64, false);
    uchar prvKeys[PRIV_GEN_MAX_BATCH * 32u];
    for (int num = 0; num < 8; ++num) {
        priv_gen_plus_variants(seed_base, num, prvKeys);
        priv_gen_process_rounds(isResult, buffResult, prvKeys, int(PRIV_THREAD_STEPS), round, precPtr,
                                precPitch, true, seed_l, nullptr, false, false, config, filters,
