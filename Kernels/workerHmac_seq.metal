...
kernel void workerHmac_seq(device bool* isResult [[buffer(0)]],
                           ...
                           const device MnwRuntimeBuffers& runtime [[buffer(11)]],
                           uint tid [[thread_position_in_grid]]) {
    FoundBuffers found;
    if (!mnw_make_found_from_runtime(found, runtime)) {
        return;
    }
    mnw_worker_seq(MNW_KIND_HMAC, isResult, buffResult, precPtr, precPitch, d_derivations,
                   derindex, der_indexes_size, der_start_index, mode, start_point_dev,
                   64, round, tid, *runtime.config, *runtime.filters,
