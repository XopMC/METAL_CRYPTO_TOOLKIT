...
kernel void workerSeed_recovery_hexset(device bool* isResult [[buffer(0)]],
                                       ...
                                       const device MnwRuntimeBuffers& runtime [[buffer(16)]],
                                       uint tid [[thread_position_in_grid]]) {
    FoundBuffers found;
    if (!mnw_make_found_from_runtime(found, runtime)) {
        return;
    }
    mnw_worker_seq_hexset(MNW_KIND_SEED, true, isResult, buffResult, precPtr, precPitch,
                          d_derivations, derindex, der_indexes_size, der_start_index,
                          hexset_start_digits, hexset_lower_exact, hexset_upper_exact,
                          hexset_alphabet, hexset_base, hexset_size, gpu_stride, round,
