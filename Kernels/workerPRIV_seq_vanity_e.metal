...
kernel void workerPRIV_seq_vanity_e(device bool* isResult [[buffer(0)]],
                                    ...
                                    const device ulong* vanityTables [[buffer(22)]],
    ...
    FoundBuffers found;
    priv_vanity_make_found(foundStrings, foundPrvKeys, foundHash160, foundLen, nullptr,
                           foundType, foundDerivations, foundDerivations2, foundPass,
                           foundPassSize, foundRound, foundSeed, resultsCount, config, found);
    priv_vanity_run_full_y_walk_static<PrivVanityKindE, false>(
        isResult, buffResult, precPtr, precPitch, mode, start_point, step,
        thread_steps_pub, startx_buf, starty_buf, vanityTables,
        vanityTables + PRIV_VANITY_TABLE_WORDS,
        vanityTables + 2u * PRIV_VANITY_TABLE_WORDS,
        vanityTables + 2u * PRIV_VANITY_TABLE_WORDS + 4u,
        config, filters, filter_storage, bloom_storage, xor_storage,
        xor_un_storage, xor_uc_storage, xor_hc_storage, found, grid_position.x,
        thread_position.x, threads_per_threadgroup.x, threadgroup_position.x);
}
